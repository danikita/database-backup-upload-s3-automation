...
2. The **Lambda** (`app.py`) calls `ssm:SendCommand` on the EC2 instance,
   passing the backup type (`FULL`/`DIFFERENTIAL`), S3 bucket, and prefix.
...
4. The script:
   ...
   - Runs `BACKUP DATABASE ... TO DISK` (FULL or `WITH DIFFERENTIAL`).
   - Runs `aws s3 cp` to upload the `.bak` file to the S3 bucket.
...
6. **S3 Lifecycle** remains the same as the RDS project, with separate
   prefixes for `full/` and `differential/`.
...
- **Transaction log backups**: just like in the RDS project, transaction log
  backups require additional configuration (FULL recovery model, their own
  schedule) and are not covered by this script.
